...
// Проверяем длину заголовка для включения анимации бегущей строки
...
// ==========================================================
// 📋 АКТУАЛЬНЫЕ ПУТИ К MP3 ФАЙЛАМ (БЕЗ ПРОБЕЛОВ И ТОЧЕК)
// ==========================================================
const musicTracks = [
  // 🟢 VOLUME ALPHA
...
// Исправленный регистр пути к дикторам
...
function prepareAudio(isFirstStart = false) {
  ...
  // Если файл всё равно словит 404, плеер не зависнет, а пойдет дальше
...
function nextTrack() {
  ...
  const rollDice = Math.random() < 0.25 // 25% шанс на появление диктора между треками
...
function togglePlay() {
  ...
  if (isPlaying.value) {
    ...
    audioRef.value.play().catch(() => {
      console.log("Кликните по интерфейсу, чтобы запустить аудиопоток.")
    })
  }
...
<style scoped>
/* ОСНОВНОЙ КОНТЕЙНЕР (Адаптирован под темную тему по умолчанию) */
...
/* КОРПУС ПЛЕЕРА - Стилизован под темную тему сайта FanCC */
.player-body { 
  width: 340px; 
  height: 360px; 
...
/* ИСПРАВЛЕНИЕ ОТРЕЗАНИЯ НАЗВАНИЯ ТРЕКА (Бегущая строка) */
...
/* Класс бегущей строки активируется автоматически, если текст длинный */
.track-title.marquee {
